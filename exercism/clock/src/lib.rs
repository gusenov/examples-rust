...
impl Clock {
    pub fn new(hours: i32, minutes: i32) -> Self {
        Self {
            hours: 0,
            minutes: 0,
        }.add_minutes(hours * MINUTES_IN_HOUR + minutes)
    }

    pub fn add_minutes(&self, minutes: i32) -> Self {
        let total_minutes = self.hours * MINUTES_IN_HOUR + self.minutes + minutes;
        let total_hours = total_minutes / MINUTES_IN_HOUR;

        let mut result_minutes = total_minutes % MINUTES_IN_HOUR;
        let mut result_hours = total_hours % HOURS_IN_DAY;

        if result_minutes < 0 {
            result_minutes += MINUTES_IN_HOUR; // MINUTES_IN_HOUR - |result_minutes|
            result_hours -= 1;
        }

        if result_hours < 0 {
            result_hours += HOURS_IN_DAY;
        }
...
// by ukolka
// 1. переводит всё в минуты;
// 2. нормализует общее количество минут за сутки;
// 3. потом раскладывает обратно на часы и минуты.
...
impl Clock {
    pub fn new(hours: i64, minutes: i64) -> Clock {
        Clock {
            minutes: (
                ((hours * HOUR + minutes) % DAY) // берём остаток (может быть отрицательным)
                + DAY) // “переводит” число в положительную область. + DAY — сдвигаем вверх, чтобы убрать минус
                % DAY // второй % DAY — возвращаем в диапазон 0…DAY−1
        }
    }
...
// by Fbartoli
// div_euclid и rem_euclid
// Обычное целочисленное деление может вести себя неудобно для отрицательных чисел.
// Например: -61 / 60 == -1
// (округление к нулю)
// Но для часов нам хочется:  -61 минут = -2 часа + 59 минут
// Именно это даёт:
// -61.div_euclid(60) == -2
// -61.rem_euclid(60) == 59
// Проверяем: -61 = (-2) * 60 + 59
